...
export class OperationStationService extends ResourceSerivce<OperationStation>
  implements Resolve<OperationStation> {
  constructor(
    apiHttpService: ApiHttpService,
    apiEndpointsService: ApiEndpointsService
  ) {
    super(apiHttpService, apiEndpointsService, environment.operationStationApi);
  }
  resolve(
    route: ActivatedRouteSnapshot,
    routerState: RouterStateSnapshot
  ): Observable<OperationStation> {
    const id = route.params["operationStationId"];
